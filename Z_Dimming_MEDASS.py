...
def record(e, path):
    ...
    arena_map = cv2.imread('map_opaque.png')
    alpha = 0.8
    ...
    filename = path + '\\' + animal_id + ' LOOMING ' + now + '.avi'
...
def looming_stimulus(color_start, win,
                     angle, distance,
                     expand_f, remain_f, pause_f):

    # Compute stimulus size in cm
    diam = compute_size(distance, angle)

    color_curr = color_start
    color_end = -0.5
    color_increment = (color_end - color_start) / (expand_f - 1)
    # print(type(color_increment))

    disk = visual.Circle(win, units='cm', fillColor=color_start, lineColor=color_start, radius=diam / 2)
    # print(type(disk.fillColor))
    # print(disk.fillColor)

    # Darken disk from color_start to color_end in ms_expand
    for frame_e in range(1, expand_f + 1):
        disk.draw()
        win.flip()
        color_curr += color_increment
        disk.fillColor = color_curr
        disk.lineColor = color_curr
...
if __name__ == '__main__':
    ...
    ms_expand = 400  # Number of milliseconds during which the disk expands
    ms_remain = 500  # Number of milliseconds during which the disk remains on screen at full size
    ms_pause = 200  # Number of milliseconds between stimuli presentations
    ...
    disk_color_start = 1  # Color of the disk

    angle = 40  # Initial size of the disk
    # angle_fin = 40  # Final size of the disk

    dist = 23  # Distance between the screen and the animal
    ...
    for wave in range(1, numWaves + 1):
        ...
        for stimulus in range(1, numStimuli + 1):
            ...
            looming_stimulus(disk_color_start, window,
                             angle, dist,
